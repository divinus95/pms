...
--SELECT public."RegisterVisitor"('Kwame', 'Adu', 'Teshie Greda Estate', '0241236978', 'Male', 2)




--select * from public."GetAllCrimeClass"()
...
--select * from public."GetAllCells"()
CREATE OR REPLACE FUNCTION public."GetAllCells"()
RETURNS TABLE(
"CellId" integer,
"CellName" text,
"RenovationIssue" text,
"isOccupied" boolean)
AS
$$
BEGIN
    ...
    FROM "Cell" cc
		WHERE cc."isOccupied" = true;
	
END
$$
  LANGUAGE plpgsql VOLATILE;
  
  --SELECT * FROM public."GetAllPrisoners"()
...
select * from public."GetAllPrisonersByGender"('Female');
CREATE OR REPLACE FUNCTION public."GetAllPrisonersByGender"("sex" text)
RETURNS TABLE(
"PrisonerId" integer,
"FirstName" text,
"LastName" text,
...
$$
  ...
  select * from public."GetAllPrisonersByCrimeType"(1)
CREATE OR REPLACE FUNCTION public."GetAllPrisonersByCrimeType"("PrisonerClassId" integer)
RETURNS TABLE(
"PrisonerId" integer,
"FirstName" text,
"LastName" text,
...
$$
  ...
  -- FUNCTION: public.UpdateCellStatus(integer, boolean)

-- DROP FUNCTION public."UpdateCellStatus"(integer, boolean);
...
--SELECT * FROM public."GetAllPrisonersInfo"();

CREATE OR REPLACE FUNCTION public."GetAllPrisonersInfo"(
	)
    RETURNS TABLE("PrisonerId" integer, 
				  "FirstName" text, 
				  "OtherName" text, 
				  "LastName" text, 
				  "Offence" text,
				  ...
				  "DateConvicted" timestamp without time zone,
				  "DateOfBirth" timestamp without time zone,
				  "ExpectedJailTerm" timestamp without time zone,
...
--select * from public."GetAllVisitorsInfo"()
CREATE OR REPLACE FUNCTION public."GetAllVisitorsInfo"(
	)
    RETURNS TABLE("VisitorId" integer, "FirstName" text, 
				  "LastName" text, "ResidentAddress" text, "Phone" text,
				  "Gender" text, "PrisonerId" integer, "PrisonerFirstName" text, 
				  "PrisonerLastName" text, "Active" boolean) 
    LANGUAGE 'plpgsql'
...
-- FUNCTION: public.GetArrivedVisitingInfo(integer)

--select * from public."GetArrivedVisitingInfo"(2);
...
-- FUNCTION: public.UpdateDepartureStatus(integer, timestamp without time zone)

-- DROP FUNCTION public."UpdateDepartureStatus"(integer, timestamp without time zone);
...
ALTER FUNCTION public."UpdateDepartureStatus"(integer, timestamp without time zone)
    OWNER TO postgres;
